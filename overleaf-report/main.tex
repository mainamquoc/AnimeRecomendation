...
\begin{longtable}{@{}p{0.12\linewidth}@{\hspace{0.8em}}p{0.24\linewidth}@{\hspace{0.8em}}p{0.32\linewidth}@{\hspace{0.8em}}p{0.22\linewidth}@{}}
  \caption{Detailed decision record for the SVD, evaluation, and serving work led by \membertwo.}\label{tab:membertwo-process} \\
    ...
    3. Protect the holdout experiment & The risk that test ratings could influence fitted parameters or configuration decisions. & Split the filtered data once into 80\% training and 20\% outer test data before fitting. Use the fixed 50-factor, 20-epoch baseline with learning rate 0.005, regularisation 0.02, and seed 42. Keep lightweight tuning disabled, so the test set is neither a tuning set nor a source of optimistic model selection. & 761,997 training ratings, 190,500 held-out ratings, and a reproducible SVD configuration. \\
    4. Measure two different questions & Outer-holdout predictions containing true and estimated ratings for each observed test item. & Compute RMSE and MAE for rating-estimation error. Separately rank each user's held-out predictions; define relevance as a true rating at least 8; require at least ten test items and one relevant item; then macro-average Precision@10 and Recall@10. This prevents a rating-error score from being mistaken for a Top-N retrieval score. & RMSE 1.2632, MAE 0.9625, Precision@10 0.6386, Recall@10 0.8255 for 4,802 eligible users. \\
...
The fixed baseline used 50 latent factors, 20 epochs, a learning rate of 0.005, regularisation of 0.02, and random state 42. Lightweight tuning was disabled; the configuration is a reproducible baseline, not a claim of exhaustive hyperparameter optimisation. The model was trained only on the 80\% outer training split (761,997 ratings); 190,500 ratings were held out for final scoring. This ordering prevents test ratings from affecting fitted parameters or configuration selection.
...
Notebook 02 reads only those cleaned outputs. It applies the fixed-seed sampling and k-core audit, creates the outer train/test split, fits Surprise SVD, calculates rating and ranking metrics, and saves evaluation charts. It then refits a serving model on the final model data, excludes previously rated anime from each candidate set, joins metadata with fallbacks, and exports ten recommendations per demonstration user. The two-stage design keeps evaluation honest while permitting the recommendation model to use all model-scope interactions.

\section{Results and evaluation}
Table~\ref{tab:metrics} reports the outer-holdout results. RMSE 1.2632 and MAE 0.9625 quantify rating-prediction error on the 1--10 scale; lower is better. Precision@10 of 0.6386 and Recall@10 of 0.8255 quantify performance only under the stated relevant-rating threshold, eligible-user rule, and observed-item candidate set. They should not be interpreted as full-catalog Top-10 performance.

\begin{table}[H]
  \centering
  \caption{SVD outer-holdout evaluation on 952,497 sampled and k-core-filtered interactions (seed 42).}
  \label{tab:metrics}
  \begin{tabular}{lr l}
    \toprule
    \textbf{Metric} & \textbf{Value} & \textbf{Direction} \\
    \midrule
    RMSE & 1.2632 & Lower is better \\
    MAE & 0.9625 & Lower is better \\
    Precision@10 & 0.6386 & Higher is better \\
    Recall@10 & 0.8255 & Higher is better \\
    \bottomrule
  \end{tabular}
\end{table}
...
The detailed predictions in Appendix~\ref{tab:case-top-ten} illustrate why a single aggregate metric cannot describe every user experience. User \texttt{10004}, with only five retained ratings, receives a highly scored but necessarily less evidence-rich list. User \texttt{42635} has 513 ratings, giving the model substantially more interaction signal, yet its highest estimated preference is 8.8350 rather than a ceiling score. The highly positive profile (user \texttt{45649}) receives ten estimates of 10.0000, while the more critical profile (user \texttt{24323}) has a highest estimate of 5.4149. These are model estimates conditional on observed ratings and the fitted score range, not statements that one user's recommendations are objectively better than another's. The broad-genre user (\texttt{65836}) demonstrates that the list can span several genres, but the latent factors themselves must not be interpreted as named genre dimensions.
...
\begin{longtable}{r p{0.64\linewidth} r}
  ...
  \addlinespace[2pt]
  1 & Ginga Eiyuu Densetsu & 9.8077 \\
  2 & Gintama & 9.6646 \\
  3 & Aria The Origination & 9.6114 \\
  4 & Gintama': Enchousen & 9.6038 \\
  5 & Gintama Movie: Kanketsu-hen -- Yorozuya yo Eien Nare & 9.5993 \\
  6 & Shingeki no Kyojin & 9.5738 \\
  7 & Katanagatari & 9.5568 \\
  8 & Gintama° & 9.5395 \\
  9 & Rainbow: Nisha Rokubou no Shichinin & 9.5193 \\
  10 & Mushishi Zoku Shou 2nd Season & 9.4905 \\
  \addlinespace[5pt]
  \multicolumn{3}{@{}l}{\textbf{Developing history --- user \texttt{10060}}} \\
  \addlinespace[2pt]
  1 & Ginga Eiyuu Densetsu & 10.0000 \\
  2 & Kimi no Na wa. & 9.6462 \\
  3 & Tengen Toppa Gurren Lagann & 9.6082 \\
  4 & Gintama° & 9.6004 \\
  5 & Gintama Movie: Kanketsu-hen -- Yorozuya yo Eien Nare & 9.5549 \\
  6 & Gintama' & 9.4975 \\
  7 & Mushishi & 9.3837 \\
  8 & Gintama': Enchousen & 9.3663 \\
  9 & Neon Genesis Evangelion: The End of Evangelion & 9.3553 \\
  10 & Fullmetal Alchemist: Brotherhood & 9.3263 \\
  \addlinespace[5pt]
  \multicolumn{3}{@{}l}{\textbf{Typical history --- user \texttt{25321}}} \\
  \addlinespace[2pt]
  1 & Ginga Eiyuu Densetsu & 9.7105 \\
  2 & Neon Genesis Evangelion: The End of Evangelion & 9.6636 \\
  3 & Aria The Origination & 9.4069 \\
  4 & Gintama': Enchousen & 9.2951 \\
  5 & Hunter x Hunter (2011) & 9.2927 \\
  6 & Code Geass: Hangyaku no Lelouch R2 & 9.1926 \\
  7 & Gintama° & 9.1624 \\
  8 & Kimi no Na wa. & 9.1436 \\
  9 & JoJo no Kimyou na Bouken (TV) & 9.1095 \\
  10 & Code Geass: Hangyaku no Lelouch & 9.0842 \\
  \addlinespace[5pt]
  \multicolumn{3}{@{}l}{\textbf{Very active --- user \texttt{42635}}} \\
  \addlinespace[2pt]
  1 & Ginga Eiyuu Densetsu & 8.8350 \\
  2 & Aria The Origination & 8.7820 \\
  3 & Serial Experiments Lain & 8.7504 \\
  4 & Gintama': Enchousen & 8.5096 \\
  5 & Gintama° & 8.4872 \\
  6 & Tengen Toppa Gurren Lagann & 8.4477 \\
  7 & Azumanga Daioh & 8.4399 \\
  8 & Monogatari Series: Second Season & 8.4074 \\
  9 & Clannad: After Story & 8.3053 \\
  10 & Seirei no Moribito & 8.3019 \\
  \addlinespace[5pt]
  ...
  4 & Top wo Nerae 2! Diebuster & 10.0000 \\
  5 & Ore no Imouto ga Konnani Kawaii Wake ga Nai Specials & 10.0000 \\
  6 & Coquelicot-zaka kara & 10.0000 \\
  7 & Aa! Megami-sama! (TV) Specials & 10.0000 \\
  8 & Bakuman. 2nd Season & 10.0000 \\
  9 & Toriko & 10.0000 \\
  10 & Kanojo to Kanojo no Neko & 10.0000 \\
  \addlinespace[5pt]
  \multicolumn{3}{@{}l}{\textbf{More critical --- user \texttt{24323}}} \\
  \addlinespace[2pt]
  1 & Mobile Suit Gundam Seed Destiny & 5.4149 \\
  2 & Sen to Chihiro no Kamikakushi & 4.7674 \\
  3 & Clannad: After Story & 4.5838 \\
  4 & Naruto: Shippuuden Movie 2 - Kizuna & 4.5431 \\
  5 & Mobile Suit Gundam Seed & 4.4712 \\
  6 & Watashi ga Motenai no wa Dou Kangaetemo Omaera ga Warui! & 4.4710 \\
  7 & Shinsekai yori & 4.4677 \\
  8 & Fairy Tail & 4.3989 \\
  9 & Gungrave & 4.3516 \\
  10 & Ginga Eiyuu Densetsu & 4.3406 \\
  \addlinespace[5pt]
  \multicolumn{3}{@{}l}{\textbf{Broad genre exposure --- user \texttt{65836}}} \\
  \addlinespace[2pt]
  1 & Cowboy Bebop & 10.0000 \\
  2 & Death Note & 10.0000 \\
  3 & Ginga Eiyuu Densetsu & 9.4676 \\
  4 & Shinsekai yori & 9.4242 \\
  5 & Steins;Gate & 9.4122 \\
  6 & Mushishi & 9.2666 \\
  7 & Hajime no Ippo & 9.1375 \\
  8 & Clannad: After Story & 9.0899 \\
  9 & Tengen Toppa Gurren Lagann & 9.0542 \\
  10 & Ping Pong The Animation & 9.0004 \\
\end{longtable}
\endgroup

\section{Conclusion and lessons learned}
This project produced a reproducible anime recommender based on explicit ratings and biased SVD. The data audit preserves the meaning of watched-but-unrated interactions, removes duplicate user--anime ambiguity, and exposes the sparse, positively skewed, long-tail structure that affects recommendation quality. On the documented sampled and filtered outer holdout, the model achieved RMSE 1.2632, MAE 0.9625, Precision@10 0.6386, and Recall@10 0.8255.
